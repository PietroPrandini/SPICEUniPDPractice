...
Applying a small alternate voltage signal to the circuit of the figure \ref{PiSimple} there aren't current on the resistance $r_0$ so it could be treated as a short circuit.\\
Given that the circuit of the figure \ref{PiSimple} is symmetrical it's possible analyse the single ended behaviour of the circuit on figure \ref{PiSingle2}.

\begin{figure}[h]
  \centering
  \includegraphics[width=12cm]{schematics/DifferentialAmplifierSmallSignalPiSingle2VID.jpg}
...
\frac{V_{D_2}}{V_{id}} = g_m {R_{D_2}} = A_{d_{single\cdot ended}}
\end{align}
